...
useHead({
  ...
  meta: [
    ...generalOg({
      ...
      image: `${config.public.supabaseUrl}/functions/v1/og-image?display_name=${attendeeDataByUserId.value?.display_name}&avatar_url=${attendeeDataByUserId.value?.avatar_url}&role=${attendeeDataByUserId.value?.role?.includes('party') ? 'party' : 'attendee'}`,
    }),
    ...twitterOg({
      title: `${conferenceTitle}`,
      description: ogSpeakerDescription,
